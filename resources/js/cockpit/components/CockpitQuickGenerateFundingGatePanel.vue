...
const checks = computed<DisplayCheck[]>(() => {
    if (!Array.isArray(props.fundingGate?.checks)) {
        return [];
    }

    return props.fundingGate.checks
        .map((check): DisplayCheck | null => normalizeCheck(check))
        .filter((check): check is DisplayCheck => check !== null);
});

const status = computed<string>(() => {
    return displayValue(props.fundingGate?.status, 'not_wired');
});

const redaction = computed<string>(() => {
    return displayValue(props.fundingGate?.redactions?.payloads, 'funding-gates-only');
});

function normalizeCheck(check: CockpitQuickGenerateFundingGateCheck): DisplayCheck | null {
    ...
    return {
        ...
        reason: displayValue(check.reason, 'No funding diagnostic is available.'),
    };
}
