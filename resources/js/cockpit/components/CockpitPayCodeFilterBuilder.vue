...
const activeFilterCount = computed(
    () => props.filters.filter((filter) => filter.active === true).length,
);
const contextFilterCount = computed(
    () =>
        props.filters.filter((filter) => filter.key.startsWith('campaign_'))
            .length,
);
...
<template>
    <section
        class="rounded-xl border border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-950"
    ...
    >
        <header>
            ...
            >
                ...
                >
                    <div
                        class="rounded-full bg-white px-3 py-1.5 ring-1 ring-slate-100 dark:bg-slate-900 dark:ring-slate-800"
                    >
                        <dt
        ...
        <p class="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-300">
            Filtering uses normal GET navigation and only changes what the
            operator sees.
        </p>
