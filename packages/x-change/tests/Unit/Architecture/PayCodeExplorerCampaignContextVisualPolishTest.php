...
it('documents pay code explorer campaign context visual polish slice 1', function (): void {
    ...
    expect($report)
        ...
        ->and($page)->toContain('Campaign filter active')
        ->and($page)->toContain('cockpit-campaign-navigation-context')
        ->and($page)->toContain('Campaign filter details')
        ->and($page)->not->toContain('{{ campaignNavigationContext.mutation?.reason }}')
        ->and($hostPage)->toContain('Campaign filter active')
        ->and($frontendTest)->toContain('Campaign filter active')
        ->and($frontendTest)->toContain('cockpit-campaign-navigation-context-item')
