...
it('demonstrates voucher liability money semantics through a no-claim lifecycle scenario', function () {
    ...
    expect($result->exitCode)->toBe(Command::SUCCESS)
        ...
        ->and(data_get($result->payload, 'money_movement_target.status'))->toBe('pending_human_approval')
        ->and(data_get($result->payload, 'money_movement_target.recommended_model'))->toBe('reserve_at_issuance_debit_at_redemption')
        ->and(data_get($result->payload, 'money_movement_target.selected_model'))->toBeNull()
        ->and(data_get($result->payload, 'money_movement_target.redactions.mutates_wallets'))->toBeFalse()
        ->and(data_get($result->payload, 'money_movement_triggers.status'))->toBe('partially_operational')
        ->and(data_get($result->payload, 'money_movement_triggers.triggers'))->toHaveCount(6)
        ->and(data_get($result->payload, 'money_movement_triggers.redactions.mutates_wallets'))->toBeTrue()
        ->and(data_get($result->payload, 'money_movement_triggers.redactions.reserves_funds'))->toBeFalse()
