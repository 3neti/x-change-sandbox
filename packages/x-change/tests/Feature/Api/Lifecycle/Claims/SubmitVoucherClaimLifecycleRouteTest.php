...
it('submits claim through redeem path via the lifecycle route surface', function () {
    ...
    $response = $this->postJson(
        '/api/x/v1/vouchers/code/'.$voucher->code.'/claim/submit',
        $payload,
    );
...
it('submits claim through withdraw path via the lifecycle route surface', function () {
    $voucher = issueVoucher(validVoucherInstructions());
...
it('returns not found when submitting claim for unknown voucher code through the lifecycle route surface', function () {
    $response = $this->postJson(
        '/api/x/v1/vouchers/code/DOES-NOT-EXIST/claim/submit',
        [
            'mobile' => '09171234567',
            'inputs' => [],
            'bank_account' => [
...
it('validates required claim submit payload fields through the lifecycle route surface', function () {
    $voucher = issueVoucher(validVoucherInstructions());
