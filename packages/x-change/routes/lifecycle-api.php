...
use LBHurtado\XChange\Lifecycle\Http\Controllers\Funding\ReceiveFundingWebhookController;
...
Route::prefix('api/x/v1')->as('api.x.v1.')->group(function (): void {
    ...
    Route::middleware(config('x-change.funding.webhook_middleware', ['throttle:120,1']))
        ->post('/funding/webhooks/{provider}', ReceiveFundingWebhookController::class)
        ->where('provider', '[a-z0-9_-]+')
        ->name('funding.webhooks.receive');
