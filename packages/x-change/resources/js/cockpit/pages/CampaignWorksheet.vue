            ...
            <section class="grid gap-5 xl:grid-cols-[minmax(0,1fr)_24rem]">
                <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    <div class="flex flex-col gap-3 border-b border-slate-200 px-4 py-3 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800">
                        <div>
                            <p class="text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">{{ isDraft() ? 'Draft Beneficiaries' : 'Authorized Beneficiaries' }}</p>
                            <h2 class="mt-0.5 font-semibold text-slate-950 dark:text-slate-50">{{ isDraft() ? 'Worksheet' : 'Authorized Worksheet' }}</h2>
                            <p v-if="isDraft()" class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ props.worksheet.rows.length === 0 ? 'Add at least one beneficiary before approval.' : hasPendingImportRows ? 'Review or discard the pending beneficiary import first.' : 'Ready to lock for officer authorization.' }}</p>
                        </div>
                        <button
                            v-if="isDraft()"
                            type="button"
                            data-testid="campaign-create-approval-pay-code"
                            :disabled="props.worksheet.rows.length === 0 || hasPendingImportRows || authorizationForm.processing"
                        ...
                        >
                            ...
                            {{ authorizationForm.processing ? 'Creating…' : 'Create Approval Pay Code' }}
                        </button>
                    </div>
