...
type Props = CockpitHeaderPageProps & { worksheet: Worksheet; imports: Import[]; fulfillment_summary: Record<string, number> };
...
<template>
    <CockpitLayout active-navigation="campaigns" :cockpit-header-read-model="props.cockpit_header_read_model">
        <main class="mx-auto max-w-7xl space-y-5" data-testid="cockpit-campaign-worksheet-page">
            ...
            <section v-if="props.worksheet.status === 'authorized' && props.worksheet.fulfillment_mode === 'pay_code_distribution'" class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p class="text-[0.65rem] font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Beneficiary Fulfillment</p><h2 class="mt-0.5 font-semibold text-slate-950 dark:text-slate-50">Issue Pay Code Batch</h2><p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Creates up to 100 beneficiary Pay Codes. Export is for external delivery; no SMS or email is sent.</p></div><div class="flex gap-2"><a :href="exports.payCodes(props.worksheet.reference).url" class="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-300 px-3 py-2.5 text-sm font-semibold text-slate-700 dark:border-slate-700 dark:text-slate-200"><Download class="size-4" /> Export</a><button type="button" :disabled="fulfillmentForm.processing" class="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60 dark:bg-slate-100 dark:text-slate-950" @click="issue"><Send class="size-4" /> {{ fulfillmentForm.processing ? 'Issuing…' : 'Issue Next 100' }}</button></div></div></section>
