...
const variance = computed(() => Number((voucherAmount.value - sliceTotal.value).toFixed(2)));
const hasBalancedSlices = computed(() =>
    !props.enabled || (voucherAmount.value > 0 && Math.abs(variance.value) < 0.01)
);
...
function emitSlices(nextSlices: PayCodeNamedSlice[]): void {
    emit('update:slices', nextSlices.map((slice, index) => ({
        ...slice,
...
function toggleEnabled(value: boolean | 'indeterminate'): void {
    ...
    if (enabled && slices.value.length === 0) {
        emitSlices([{
            id: 'slice_1',
...
function updateSlice(index: number, key: keyof PayCodeNamedSlice, value: unknown): void {
...
function splitEvenly(): void {
    ...
    emitSlices(Array.from({ length: count }, (_, index) => {
        ...
        return {
            ...
            description: count === 1 ? 'Whole amount' : `Slice ${index + 1}`,
            tag: '',
            claim_on: '',
            claim_by: '',
        };
...
<template>
    <div class="space-y-4 rounded-xl border bg-muted/10 p-4">
        <label class="flex items-start justify-between gap-4">
            <div class="space-y-1">
                ...
                <p class="text-xs text-muted-foreground">
                    Define exact claim units with issuer-facing descriptions and claim windows.
                </p>
            </div>
        ...
        <div v-if="props.enabled" class="space-y-4">
            <div class="flex flex-wrap items-center justify-between gap-3 rounded-lg border bg-background p-3">
                <div class="flex flex-wrap items-center gap-2 text-sm">
                    <Badge variant="secondary">
                        Voucher ₱{{ voucherAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}
                    </Badge>
                    <Badge :variant="hasBalancedSlices ? 'default' : 'destructive'">
                        Slices ₱{{ sliceTotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}
                    </Badge>
                    <span v-if="!hasBalancedSlices" class="text-xs text-destructive">
                        Difference ₱{{ Math.abs(variance).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}
                    </span>
                </div>

                <div class="flex flex-wrap gap-2">
                    <Button type="button" variant="outline" size="sm" @click="useWholeAmount">
                        Whole amount
                    </Button>
                    <Button type="button" variant="outline" size="sm" @click="splitEvenly">
                        Split evenly
                    </Button>
                </div>
            </div>
                    ...
                    <div class="grid gap-3 sm:grid-cols-2">
                        <div class="space-y-2">
                            ...
                            <Input
                                ...
                                @update:model-value="updateSlice(index, 'amount', $event)"
                            />
                        </div>

                        <div class="space-y-2">
                            <Label :for="`slice-${index}-description`">Description</Label>
                            <Input
                                :id="`slice-${index}-description`"
                                :placeholder="slices.length === 1 ? 'Whole amount' : `Slice ${index + 1}`"
                                :model-value="slice.description ?? ''"
                                @update:model-value="updateSlice(index, 'description', $event)"
                            />
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <Input
                                ...
                                @update:model-value="updateSlice(index, 'tag', $event)"
                            />
                        </div>

                        <div class="grid gap-3 sm:grid-cols-2">
                            <div class="space-y-2">
                                <Label :for="`slice-${index}-claim-on`">Claim on</Label>
                                <Input
                                    :id="`slice-${index}-claim-on`"
                                    type="datetime-local"
                                    :model-value="slice.claim_on ?? ''"
                                    @update:model-value="updateSlice(index, 'claim_on', $event)"
                                />
                            </div>

                            <div class="space-y-2">
                                <Label :for="`slice-${index}-claim-by`">Claim by</Label>
                                <Input
                                    :id="`slice-${index}-claim-by`"
                                    type="datetime-local"
                                    :model-value="slice.claim_by ?? ''"
                                    @update:model-value="updateSlice(index, 'claim_by', $event)"
                                />
                            </div>
                        </div>
                    </div>
            ...
            <Button type="button" variant="outline" class="w-full" @click="addSlice">
