...
Current status: Wave 5 — x-campaign complete through Phase 15; x-change read-only Cockpit visual validation passed; Quick Generate mutation Wave 1F closed; published asset drift guard complete; Cockpit Mutation Wave 2D action handoff boundary implemented
...
| 4 | x-change Cockpit | Operator shell | Mutation Wave 2D action handoff boundary implemented | [../ui-cockpit/COMPASS.md](../ui-cockpit/COMPASS.md) |
...
```text
Approve or revise Cockpit Mutation Wave 2E — Feedback Handoff Boundary.
```
...
15. Approve, revise, or reject Wave 2E before a feedback handoff boundary is scaffolded.
16. Keep package access adapter-driven inside x-change; do not duplicate integration wiring in the host app.
17. Existing issuance handoff, idempotency/replay, UI submit, manual refresh, and generated Pay Code detail navigation are wired.
18. Do not add campaign mutation endpoints, delivery dispatch, execution, journal writes, action execution, feedback delivery, provider calls outside `GeneratePayCode`, campaign state mutation, direct money movement, raw payload exposure, or direct wallet access unless explicitly approved.
19. Keep Claim UI protected and keep all productized Cockpit work inside `packages/x-change`.
