...
Current status: Wave 5 — x-campaign complete through Phase 15; x-change read-only Cockpit visual validation passed; Quick Generate mutation Wave 1F closed; published asset drift guard complete; Cockpit Mutation Wave 2E feedback handoff boundary implemented
...
| 4 | x-change Cockpit | Operator shell | Mutation Wave 2E feedback handoff boundary implemented | [../ui-cockpit/COMPASS.md](../ui-cockpit/COMPASS.md) |
...
```text
Approve or revise Cockpit Mutation Wave 2F — Activity Presentation Closure.
```
...
16. Approve, revise, or reject Wave 2F before activity presentation closure is scaffolded.
17. Keep package access adapter-driven inside x-change; do not duplicate integration wiring in the host app.
18. Existing issuance handoff, idempotency/replay, UI submit, manual refresh, and generated Pay Code detail navigation are wired.
19. Do not add campaign mutation endpoints, delivery dispatch, execution, journal writes, action execution, feedback delivery, provider calls outside `GeneratePayCode`, campaign state mutation, direct money movement, raw payload exposure, or direct wallet access unless explicitly approved.
20. Keep Claim UI protected and keep all productized Cockpit work inside `packages/x-change`.
